...
exports.compile = function(root, libs) {
    ...
    // Fonts.
    var urlPrefix = Path.join(libs.getBackToRoot(libs.dirname()), "..");
    libs.addInnerCSS(
        "@font-face{font-family:fira;src: url("
            + Path.join(urlPrefix, "FiraSans-Light.ttf").replace(/\\/g, '/')
            + ")}"
    );
    libs.addResourceFile(
        'FiraSans-Light.ttf',
        Path.join('FiraSans-Light.ttf'),
        Path.join('com', 'x-article', 'FiraSans-Light.ttf')
    );
    libs.addInnerCSS(
        "@font-face{font-family:fira;src: url("
            + Path.join(urlPrefix, "FiraSans-Regular.ttf").replace(/\\/g, '/')
            + ");font-weight:bold}"
    );
    libs.addResourceFile(
        'FiraSans-Regular.ttf',
        Path.join('FiraSans-Regular.ttf'),
        Path.join('com', 'x-article', 'FiraSans-Regular.ttf')
    );
    ...
    if (result.length > 1) {
        result.forEach(function (page, idx) {
            ...
            for (var idxPage = 0 ; idxPage < result.length ; idxPage++) {
                ...
                footer += "<span>";
                if (idx == idxPage) {
                    footer += "<span class='selected'>" + (1 + idx) + "</span>";
                } else {
            ...
            footer += "</footer>";
            page.$article.children.push(libs.parseHTML(footer));
        });
    }
